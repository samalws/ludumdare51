...
var TextObj = /** @class */ (function () {
    function TextObj(text, yPos, color) {
        this.text = text;
        this.yPos = yPos;
        this.color = color;
    }
    TextObj.prototype.render = function (ctx) {
        ctx.font = "bold 48px sans-serif";
        ctx.fillStyle = this.color;
        ctx.fillText(this.text, 10, this.yPos);
    };
    TextObj.prototype.update = function (delta) { };
    TextObj.orangeColor = "#E58F65";
    TextObj.redColor = "#CA1551";
    TextObj.greenColor = "#A2FAA3";
    return TextObj;
}());
var ScoreObj = /** @class */ (function (_super) {
    __extends(ScoreObj, _super);
    function ScoreObj() {
        return _super.call(this, "0", 50, TextObj.orangeColor) || this;
    }
...
var GameObject = /** @class */ (function () {
    function GameObject(pos, vel, imgName, particleSpeed) {
        if (particleSpeed === void 0) { particleSpeed = 300 / 1000; }
    ...
    GameObject.prototype.update = function (delta) {
        ...
        if (this.timeToParticle <= 0) {
            ...
            if (velMagSq > epsilon) {
                ...
                var particleVel = mulVec(negVelUnit, this.particleSpeed);
...
var Enemy = /** @class */ (function (_super) {
    ...
    function Enemy(pos, vel, imgName, particleSpeed) {
        if (particleSpeed === void 0) { particleSpeed = 300 / 1000; }
        var _this = _super.call(this, pos, vel, imgName, particleSpeed) || this;
...
var ProtoBasicEnemy = /** @class */ (function (_super) {
    ...
    function ProtoBasicEnemy(pos, targetPos) {
        var _this = _super.call(this, pos, originVec, "protoBasicEnemy", 0) || this;
        _this.timeToUpdate = 0;
...
function update(delta) {
    ...
    if (timeToEnemySpawn <= 0) {
        ...
        if (score == 0)
            ...
            var numSpawns = (score < scoreCutoff1) ? 2 : (score < scoreCutoff2) ? 3 : 4;
...
function togglePause() {
    ...
    if (gameIsPaused) {
        ...
        pauseMenu.forEach(function (e) { return removeFromList(e, objectList); });
    }
}
function initGame() {
    player = new Player();
...
initTut();
function checkNullable(x) {
    if (x === null)
        throw "null value";
    return x;
}
var canvas = document.getElementById("canvas");
var realCanvasSize = Math.min(window.innerWidth, window.innerHeight) * .9;
canvas.width = realCanvasSize;
canvas.height = realCanvasSize;
var scaleFactor = realCanvasSize / canvasSize;
var context = checkNullable(canvas.getContext("2d"));
...
function toggleSfx() {
    ...
    if (!sfxEnabled) {
        engineAudio.pause();
        Object.values(sfxAudio).forEach(function (l) { return l.forEach(function (a) { return a.pause(); }); });
    }
}
window.setInterval(function () { return console.log("max render time:", maxRenderTime, ", max update time:", maxUpdateTime); }, 1000);
