...
var Background = /** @class */ (function () {
    ...
    Background.prototype.render = function (ctx, xSize, ySize) {
        ...
        Background.stars.forEach(function (s) { return ctx.fillRect(s.x, s.y, 2, 2); });
    };
...
function update(delta) {
    ...
    if (timeToEnemySpawn <= 0) {
        ...
        var numSpawnsDec = Math.floor(Math.random() * 2);
        for (var i = 0; i <= numSpawnsDec; i++) {
            var rng = Math.random();
            if (rng < 1 / 7)
                spawnEnemy(wizEnemy);
            else if (rng < 2 / 7)
